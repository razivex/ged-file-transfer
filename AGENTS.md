...
2. **Single secrets file:** `ged-file_transfer/.env` (gitignored). Do not put real passwords in `.env.example`.
...
```text
ged-file_transfer/          ← transfer.py, .env, requirements.txt
...
2. Otherwise run the GED lookup. If it returns a non-empty name → rename to that name (keep the original extension when the query result has none) and move to destiny.
